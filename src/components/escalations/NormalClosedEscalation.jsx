...
function App() {
    ...
    const showDetails = (requestId) => {
        ...
        const type = Base64.encode(2);
        navigate(`/show-closed-escalation-details/${encodedId}/${type}`);
    ...
    const columns = [
        {
            ...
            sortable: true,
            cell: row => <div className="wrap-text cursor-pointer">{row.agency_name}</div>,
        },
        {
            name: 'Raised By',
            sortable: true,
        ...
        },
        {
            name: 'Closed By',
            sortable: true,
            cell: row => <div className="wrap-text">{row.updated_by}</div>,
        },
        {
            name: 'Raised Date',
        ...
        },
        {
            ...
            sortable: true,
            cell: row => <div className="wrap-text">{row.updated_date}</div>,
        },
        {
            name: 'Total Escalations',
            selector: row => row.duplicacy_count,
            sortable: true,
            cell: row => <div onClick={() => showDetails(row.agency_id)} className="  cursor-pointer wrap-text bg-green-500 rounded-full text-white text-center flex items-center justify-center" style={{
                minWidth: '2rem',
                minHeight: '2rem',
