...
const WaiverDetails = () => {
    ...
    const getWaiverDetails = async () => {
        ...
        try {
            ...
            setPrincipalPercentage(customerData.principal_amt == 0 ? 0 : calculatePercentage(customerData.principal_amt, customerData.approved_principal));

            setIntrestPercentage(customerData.intrest_amt == 0 ? 0 : calculatePercentage(customerData.intrest_amt, customerData.approved_intrest));

            setPenalPercentage(customerData.penal_amt == 0 ? 0 : calculatePercentage(customerData.penal_amt, customerData.approved_penal));
                            ...
                            <div className="space-y-2">
                                <div className="flex flex-wrap">
                                    ...
                                    <div className="w-full flex mb-2">
                                        ...
                                        <p className="w-1/2">{waiverData.overdue_int ? waiverData.overdue_int.toFixed(2) : "-"}</p>
                                    </div>
                                    <div className="w-full flex mb-2">
                                        <p className="font-semibold w-1/2">DOB:</p>
                                        <p className="w-1/2">{waiverData.dob ? waiverData.dob : "-"}</p>
                                    </div>
                                    <div className="w-full flex mb-2">
                                        <p className="font-semibold w-1/2">AGE:</p>
                                        <p className="w-1/2">{waiverData.age ? waiverData.age : "-"}</p>
                                    </div>
                                    <div className="w-full flex mb-2">
                                        <p className="font-semibold w-1/2">Phone:</p>
                                        <p className="w-1/2">{waiverData.phone_number ? waiverData.phone_number : "-"}</p>
                                    </div>

                                    <div className="w-full flex mb-2">
                                        <p className="font-semibold w-1/2">Last EMI:</p>
                                        <p className="w-1/2">{waiverData.last_emi_date ? waiverData.last_emi_date : "-"}</p>
                                    </div>
                                    <div className="w-full flex mb-2">
                                        <p className="font-semibold w-1/2">Last payment:</p>
                                        <p className="w-1/2">{waiverData.date_of_last_payment ? waiverData.date_of_last_payment : "-"}</p>
                                    </div>
                                    <div className="w-full flex mb-2">
                            ...
                            <div className="space-y-2">
                                <div className="flex flex-wrap">
                                    <div className="w-full flex mb-2">
                                        ...
                                        <p className="w-1/2">{waiverData.agencyDetails.nbfc_name}</p>
                                    </div>
                                    <div className="w-full flex mb-2">
                                    ...
                                    <div className="w-full flex mb-2">
                                        ...
                                        <p className="w-1/2"> <ChangeDateFormate date={waiverData.created_date ? waiverData.created_date : "-"} /></p>
                                    </div>
                                    <div className="w-full flex mb-2">
                                    ...
                                    <div className="w-full flex mb-2">
                                        ...
                                        <p className="w-1/2">{getBucketName(waiverData.PoolData.bucket)}</p>
                                    </div>
                                    <div className="w-full flex mb-2">
                        ...
                        <div className="w-full p-6 bg-white rounded-lg shadow-md">
                            ...
                            <form className="space-y-4" onSubmit={HandleSubmit}>
                                <div className="flex items-center">
                                    ...
                                    <input disabled={true} value={principalReq} className="w-1/4 p-2 border rounded" type="text" />
                                    <label className="w-3/4" htmlFor="name">Max Allowed (%):</label>
                                    <input disabled={true} className="w-1/4 p-2 border rounded" type="text" value={principalRule} />
                                    <label className="w-3/4" htmlFor="name">Waiver in (%):</label>
                                    <input disabled={true} value={principalPercentage} className="w-1/4 p-2 border rounded" type="text" />
                                    <label className="w-3/4" htmlFor="name">Principal Approved (₹):</label>
                                    <input value={updatedPrincipal} onChange={HandlePrincipal} className="w-1/4 p-2 border rounded" type="number" />
                                </div>
                                <div className="flex items-center">
                                    <label className="w-3/4" htmlFor="name">Penal Request (₹):</label>
                                    <input disabled={true} value={penalReq} className="w-1/4 p-2 border rounded" type="text" />
                                    <label className="w-3/4" htmlFor="name">Max Allowed (%):</label>
                                    <input disabled={true} value={penalRule} className="w-1/4 p-2 border rounded" type="text" />
                                    <label className="w-3/4" htmlFor="name">Waiver in (%):</label>
                                    <input disabled={true} value={penalPercentage} className="w-1/4 p-2 border rounded" type="text" />
                                    <label className="w-3/4" htmlFor="name">Penal Approved (₹):</label>
                                    <input value={updatedPenal} onChange={HandlePenal} className="w-1/4 p-2 border rounded" type="number" />
                                </div>

                                <div className="flex items-center">
                                    <label className="w-3/4" htmlFor="name">Interest Request (₹):</label>
                                    <input disabled={true} value={intrestReq} className="w-1/4 p-2 border rounded" type="text" />
                                    <label className="w-3/4" htmlFor="name">Max Allowed (%):</label>
                                    <input disabled={true} value={intrestRule} className="w-1/4 p-2 border rounded" type="text" />
                                    <label className="w-3/4" htmlFor="name">Waiver in (%):</label>
                                    <input disabled={true} value={intrestPercentage} className="w-1/4 p-2 border rounded" type="text" />
                                    <label className="w-3/4" htmlFor="name">Interest Approved (₹):</label>
                                    <input value={updatedIntrest} onChange={HandleIntrest} className="w-1/4 p-2 border rounded" type="number" />
                                </div>
                                <div className="flex items-center">

                                    <textarea value={reason} onChange={handleReason} className="w-full p-2 border rounded" type="textarea" placeholder='Enter the reason for approval / rejection the waiver' />
