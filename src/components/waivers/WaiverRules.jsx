...
function App() {
    ...
    const getExpiryStatus = (expiry_date) => {
        ...
        const formattedDate = expiry_date.replace(/(\d{2})-(\d{2})-(\d{4}) (\d{2}):(\d{2}):(\d{2}) (AM|PM)/, '$2-$1-$3 $4:$5:$6 $7');
    ...
    const columns = [
        {
            ...
            sortable: true,
            cell: row => <div onClick={() => handleAction(row.product)} className="wrap-text cursor-pointer">{row.product}</div>,
        },
        {
            name: 'Bucket',
        ...
        },
        {
            name: 'Status',
            selector: row => row.formatted_expiry_date,
            cell: row => <StatusButton status={getExpiryStatus(row.formatted_expiry_date)} />,
