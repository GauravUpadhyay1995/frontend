...
const NestedTable = () => {
  ...
  const getData = async () => {
    try {
      const res = await axios.post(`/api/commercial/listCommercialRule`, {
        body: JSON.stringify({}),
      });
  ...
  const ExpandedComponent = ({ data }) => {
    ...
    return (
      <div className="ml-20 border-solid border-9 mb-1 mt-1">
        <DataTable columns={nestedColumns} data={data.Slabs} noHeader />
      </div>
    );
  };
  ...
  const columns = [
    ...
    },
    {
      ...
      selector: (row) => row.fixed_percentage + "%",
      sortable: true,
    },
